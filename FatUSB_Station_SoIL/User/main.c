...
// 全局变量
volatile Send4GState SoilDataState = DATA_PREPARE;						// 数据包上传标志位
...
int main(void)
{
  ...
  uart_init(115200);                													//4G串口初始化
	...
	while (1)
	{
			...
			if(SoilDataState == DATA_READY) {   									//数据包上传标志判断
				SoilDataState = DATA_PREPARE;											//清空标志位
				if(StateOfHarfWare == ALL_OPEN){
				IWDG_ReloadCounter();																	//喂狗，给4G充足的发送时间防止信号不好重启
				SoilData2Web(AdressID,PassWord);										//发送数据到平台	
				}
		...
		if(StateOfHarfWare != CLOSE_ALL ){																//判断状态RS485未关闭
			...
			{
				case 0:   SoilCheck_Cmd1(); IWDG_ReloadCounter();break;					
																											
				case 40:  SoilCheck_Cmd2(); IWDG_ReloadCounter(); break;			
	
				case 80: SoilCheck_Cmd3(); IWDG_ReloadCounter();break;				
			}
		}
		else{																				//判断状态RS485关闭
			if(Timecount %40 == 0){										
